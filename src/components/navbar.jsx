...
function NavBar() {
  ...
  return (
    <Navbar expand="md" className="px-6  sticky z-[999] py-2 md:px-20 md:py-1 bg-customGray font-neue" expanded={isOpen}>
      <Container fluid className="d-flex justify-between px-0">
        <Navbar.Brand href="#">
        ...
        <Navbar.Toggle  style={togglerStyle} onClick={() => setIsOpen(!isOpen)} aria-controls="basic-navbar-nav">
          <svg  className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path>
          </svg>
        </Navbar.Toggle>
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="ml-auto gap-10">
            {["Services", "Our Work", "About Us", "Insites", "Contact"].map((item, index) => (
              <Nav.Link href="#" key={index} className={`text-md capitalize font-light font-neue ${index === 4 && "md:ml-32"}`}>
                {item}
              </Nav.Link>
            ))}
          </Nav>
